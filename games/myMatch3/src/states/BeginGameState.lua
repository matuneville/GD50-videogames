...
function BeginGameState:init()
    ...
    self.board = Board(VIRT_WIDTH - 272, 16)
...
function BeginGameState:enter(def)
    ...
    :finish(function()
        Timer.tween(0.25, {
            [self] = {levelLabelY = VIRT_HEIGHT / 2 - 8}
        })
        ...
        :finish(function()
            Timer.after(1, function()
                ...
                Timer.tween(0.25, {
                    [self] = {levelLabelY = VIRT_HEIGHT + 30}
                })
...
function BeginGameState:render()
    ...
    love.graphics.rectangle('fill', 0, self.levelLabelY - 8, VIRT_WIDTH, 48)
    love.graphics.setColor(1, 1, 1, 1)
    love.graphics.setFont(gFonts['large'])
    love.graphics.printf('Level ' .. tostring(self.level),
        0, self.levelLabelY, VIRT_WIDTH, 'center')
