...
function PlayerFallingState:update(dt)
    ...
    local colBottomLeft = self.player.map:pointToTile(self.player.x + 1, self.player.y + self.player.height)
    local colBottomRight = self.player.map:pointToTile(self.player.x + self.player.width - 1, self.player.y + self.player.height)

    local colY = colBottomLeft.y - colBottomLeft.height * gHeightTileWorld

    -- if we get a collision beneath us, go into either walking or idle
    if (colBottomLeft and colBottomRight) and  self.player.y + self.player.height >= colY then
        self.player.dy = 0
        ...
        self.player.y = colY - self.player.height
